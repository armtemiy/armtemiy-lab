...
const AdminModule = lazy(() =>
  import("./modules/AdminModule").then((m) => ({ default: m.AdminModule })),
);
const AnthroModule = lazy(() =>
  import("./modules/AnthroModule").then((m) => ({ default: m.AnthroModule })),
);
const CounterModule = lazy(() =>
  import("./modules/CounterModule").then((m) => ({ default: m.CounterModule })),
);
const DiagnosticWizard = lazy(() =>
  import("./modules/DiagnosticWizard").then((m) => ({
    default: m.DiagnosticWizard,
  })),
);
const PeriodizationCalculator = lazy(() =>
  import("./modules/PeriodizationCalculator").then((m) => ({
    default: m.PeriodizationCalculator,
  })),
);
const SparringPage = lazy(() =>
  import("./pages/SparringPage").then((m) => ({ default: m.SparringPage })),
);
const SparringMyProfilePage = lazy(() =>
  import("./pages/SparringMyProfilePage").then((m) => ({
    default: m.SparringMyProfilePage,
  })),
);
const SparringProfilePage = lazy(() =>
  import("./pages/SparringProfilePage").then((m) => ({
    default: m.SparringProfilePage,
  })),
);
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    const hasLaunchParams = Array.from(params.keys()).some((key) =>
    ...
    const applyTheme = (nextScheme?: string) => {
      const fallback = window.matchMedia("(prefers-color-scheme: dark)").matches
        ...
        : "light";
      const scheme =
        nextScheme === "light" || nextScheme === "dark" ? nextScheme : fallback;
              ...
              {isAdmin && (
                <div className="card">
                  ...
                  <p className="mt-2 text-xs text-faint">
                    Дерево:{" "}
                    {treeSource === "override" ? "кастомное" : "по умолчанию"} ·
                    Узлов: {Object.keys(tree.nodes).length}
                  </p>
                </div>
              )}
            </motion.div>
    ...
    <AnimatePresence mode="wait">
      ...
      >
        <Routes location={location} key={location.pathname}>
          ...
          <Route
            path="/sparring/my-profile"
            element={<SparringMyProfilePage />}
          />
          <Route
            path="/sparring/profile/:id"
            element={<SparringProfilePage />}
          />
          <Route path="*" element={<HomePage />} />
        </Routes>
      </Suspense>
